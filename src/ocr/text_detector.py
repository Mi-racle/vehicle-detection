...
from ocr.operators import perform_operators, create_operators
from ocr.postprocess import build_post_process
from ocr.config import analysis_config
...
class TextDetector(BaseOCRV20):
    def __init__(self, args, **kwargs):
        ...
        if self.det_algorithm == "DB":
            ...
            postprocess_params["scales"] = [int(v) for v in args.scales.split(',')]
        ...
        network_config = analysis_config(self.weights_path, self.yaml_path)
        super().__init__(network_config, **kwargs)
    ...
    def __call__(self, img):
        ori_im = img.copy()
        data = {'image': img}
        data = perform_operators(data, self.preprocess_op)
        img, shape_list = data
        if img is None:
            return None, 0
