...
from ocr.networks.activation import Activation
...
class LocalizationNetwork(nn.Module):
    def __init__(self, in_channels, num_fiducial, loc_lr, model_name):
        ...
        self.fiducial = num_fiducial
        fiducial = num_fiducial
        if model_name == "large":
    ...
    def forward(self, x):
        ...
        x = nn.ReLU()(x)
        x = self.fc2(x)
        x = x.reshape(shape=[-1, self.fiducial, 2])
        return x
...
class TPS(nn.Module):
    ...
    def forward(self, image):
        ...
        if torch.__version__ < '1.3.0':
            batch_I_r = torch.nn.functional.grid_sample(image, grid=batch_P_prime)
        else:
            batch_I_r = torch.nn.functional.grid_sample(image, grid=batch_P_prime, align_corners=True)
        return batch_I_r
