...
def conv3x3_block(in_channels, out_channels, stride=1):
    conv2d = nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=stride, padding=1, bias=True)
    block = nn.Sequential(conv2d, nn.BatchNorm2d(out_channels), nn.ReLU())
    return block
...
class STN_ON(nn.Module):
    def __init__(self, in_channels, tps_inputsize, tps_outputsize, num_control_points, tps_margins, stn_activation):
        ...
        self.tps = TPSSpatialTransformer(tuple(tps_outputsize), num_control_points, tuple(tps_margins))
        self.stn_head = STN(in_channels, num_control_points, stn_activation)
