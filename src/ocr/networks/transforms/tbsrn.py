...
from ocr.networks.heads.sr_rensnet_transformer import Transformer, LayerNorm, \
...
class TBSRN(nn.Module):
    def __init__(
        ...
        block_: list = [UpsampleBLock(2 * hidden_units, 2) for _ in range(upsample_block_num)]
        ...
        if self.stn:
            ...
            self.stn_head = STN(in_planes, num_control_points, 'none')
