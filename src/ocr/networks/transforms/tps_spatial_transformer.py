...
def grid_sample(_input, grid, canvas=None):
    _input.stop_gradient = False
    if torch.__version__ < '1.3.0':
        output = torch.nn.functional.grid_sample(_input, grid=grid)
    else:
        output = torch.nn.functional.grid_sample(_input, grid=grid, align_corners=True)
    if canvas is None:
        return output
    else:
        # input_mask = paddle.ones(shape=input.shape)
        input_mask = _input.data.new(_input.size()).fill_(1)
        output_mask = torch.nn.functional.grid_sample(input_mask, grid=grid)
...
class TPSSpatialTransformer(nn.Module):
    ...
    def forward(self, _input, source_control_points):
        ...
        grid = 2.0 * grid - 1.0
        output_maps = grid_sample(_input, grid, canvas=None)
